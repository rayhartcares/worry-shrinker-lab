...
/* ── State ───────────────────────────────────────────────── */
const STATE_KEY = 'wsl_v2_session';
...
function clearState() {
  ...
  Object.assign(state, {
    name: '', feeling: '', worryBefore: 5, worryText: '',
    scenarioAnswers: {}, selectedTool: '', worryAfter: 5,
    reflection: '', sessionDate: '',
  });
}
...
function initComplete() {
  ...
  const rows = [
    ...
    { label: 'Tool used',     value: tool ? tool.title : '—' },
...
function buildParentSummary() {
  ...
  container.innerHTML = `
    ...
    <div class="summary-row"><span class="summary-label">Coping tool used</span><span class="summary-value">${tool ? tool.title : '—'}</span></div>
...
function boot() {
  ...
  if (state.name && state.worryText) {
    ...
    // If resuming, determine last screen reached
    else {
      let lastScreen = 'screen-story';
      if (state.reflection)    lastScreen = 'screen-complete';
      else if (state.selectedTool) lastScreen = 'screen-reflection';
      else if (Object.keys(state.scenarioAnswers).length > 0) lastScreen = 'screen-tools';
      else if (state.worryText)    lastScreen = 'screen-scenarios';
      else if (state.feeling)      lastScreen = 'screen-write';
